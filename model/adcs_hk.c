...
adcs_hk_t *adcs_hk_parseFromJSON(cJSON *adcs_hkJSON){
    ...
    if (acs_mode_active) { 
    if(!cJSON_IsString(acs_mode_active))
    {
    goto end; //String
    }
    }
    ...
    if (acs_mode_cmd) { 
    if(!cJSON_IsString(acs_mode_cmd))
    {
    goto end; //String
    }
    }
    ...
    adcs_hk_local_var = adcs_hk_create (
        ...
        acs_mode_active ? strdup(acs_mode_active->valuestring) : NULL,
        ...
        acs_mode_cmd ? strdup(acs_mode_cmd->valuestring) : NULL,
